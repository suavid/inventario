...
function function_checkdate($checkdate, $action) {
    if ($checkdate != '') {
        ...
        if (strlen($expldate[0]) == 1) {
            $expldate[0] = "0" . $expldate[0];
            $combine = '1';
        }
        if (strlen($expldate[1]) == 1) {
            $expldate[1] = "0" . $expldate[1];
            $combine = '1';
        }
        if (strlen($expldate[2]) == 1) {
            $expldate[2] = "200" . $expldate[2];
            $combine = '1';
        }
        if (strlen($expldate[2]) == 2) {
            $expldate[2] = "20" . $expldate[2];
            $combine = '1';
        }
        if ($expldate[2] == '') {
            $expldate[2] = date("Y");
        ...
        if ($combine == '1') {
            $checkdate = $expldate[0] . "/" . $expldate[1] . "/" . $expldate[2];
        }
