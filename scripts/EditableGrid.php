...
class EditableGrid {
    ...
    public function getXML($rows, $customRowAttributes = array(), $encodeCustomAttributes = false) {
        ...
        if (!$fetchMethod)
            foreach ($rows as $row)
                $xml.= $this->getRowXML($row, $customRowAttributes, $encodeCustomAttributes);
        else
            while ($row = call_user_func(array($rows, $fetchMethod)))
                $xml.= $this->getRowXML($row, $customRowAttributes, $encodeCustomAttributes);
    ...
    private function getRowXML($row, $customRowAttributes, $encodeCustomAttributes) {
        ...
        foreach ($customRowAttributes as $name => $field)
            $xml.= " {$name}='" . ($encodeCustomAttributes ? base64_encode($this->_getRowField($row, $field)) : self::escapeXML($this->_getRowField($row, $field))) . "'";
        $xml.= ">\n";
    ...
    public function renderXML($rows, $customRowAttributes = array(), $encodeCustomAttributes = false) {
        ...
        echo trim($this->getXML($rows, $customRowAttributes, $encodeCustomAttributes));
    }
    ...
    public function getJSON($rows, $customRowAttributes = array(), $encodeCustomAttributes = false) {
        ...
        if (!$fetchMethod)
            foreach ($rows as $row)
                $data[] = $this->getRowJSON($row, $customRowAttributes, $encodeCustomAttributes);
        else
            while ($row = call_user_func(array($rows, $fetchMethod)))
                $data[] = $this->getRowJSON($row, $customRowAttributes, $encodeCustomAttributes);
    ...
    private function getRowJSON($row, $customRowAttributes, $encodeCustomAttributes) {
        ...
        foreach ($customRowAttributes as $name => $field)
            $data[$name] = $encodeCustomAttributes ? base64_encode($this->_getRowField($row, $field)) : $this->_getRowField($row, $field);
    ...
    protected function parseColumnType($type, $unitTranslations = FALSE) {
        ...
        if ($info['unit'] && isset($unitTranslations[$info['unit']]))
    ...
    public static function format($value, $columnType, $unitTranslations = FALSE) {
        ...
        if (!isset($info['datatype']) || !in_array($info['datatype'], array('double', 'integer')))
            return $value;
        ...
        if ($info['precision'] === -1)
            $displayValue = trim(number_format($displayValue, 8, $info['decimal_point'], $info['thousands_separator']), "0");
        else if ($info['precision'] !== NULL)
            $displayValue = number_format($displayValue, $info['precision'], $info['decimal_point'], $info['thousands_separator']);
        if ($displayValue[strlen($displayValue) - 1] == '.')
            $displayValue = substr($displayValue, 0, strlen($displayValue) - 1);

        if ($info['unit'])
            $displayValue = $info['unit_before_number'] ? ($info['unit'] . ' ' . $displayValue) : ($displayValue . ' ' . $info['unit']);
