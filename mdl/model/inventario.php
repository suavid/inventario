...
class inventarioModel extends object {
    ...
    public function consultar_inventario($tipoQuery, $bodega, $Li, $Ls, $Pi, $Ps, $estilo) {
        import('scripts.periodos');
        $pf = "";
        $pa = "";
        $estilo_str = "";
        if(trim($estilo)!=""){
            $estilo_str = " AND articulo.estilo ='{$estilo}'";    
        }
        ...
        if ($bodega > 0) {
            switch ($tipoQuery) {
                case 1:
                    $query = "SELECT  linea.id as id_linea, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) WHERE bodega.id = $bodega AND  (articulo.linea >= $Li AND articulo.linea<= $Ls) $estilo_str GROUP BY linea.id ORDER BY no DESC";
                    break;
                case 2:
                    $query = "SELECT  linea.id as id_linea, proveedor.nombre as proveedor, proveedor.id as id_proveedor, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id WHERE bodega.id = $bodega AND (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id ORDER BY no DESC";
                    break;
                case 3:
                    $query = "SELECT  articulo.estilo as estilo, linea.id as id_linea, proveedor.nombre as proveedor,proveedor.id as id_proveedor, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id WHERE bodega.id = $bodega AND (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id, articulo.estilo ORDER BY no DESC";
                    break;
                case 4:
                    $query = "SELECT  color.id as id_color,color.nombre as color, articulo.estilo as estilo, linea.id as id_linea, proveedor.nombre as proveedor, proveedor.id as id_color, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id LEFT JOIN color ON articulo.color = color.id WHERE bodega.id = $bodega AND (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id, articulo.estilo, articulo.color ORDER BY no DESC;";
                    break;
                case 5:
                    $query = "SELECT  exi_costo_unitario as costo, articulo.talla as talla, color.nombre as color, color.id as id_color, articulo.estilo as estilo, linea.id as id_linea, proveedor.nombre as proveedor,proveedor.id as id_proveedor, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id LEFT JOIN color ON articulo.color = color.id WHERE bodega.id = $bodega AND (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id, articulo.estilo, articulo.color, articulo.talla ORDER BY no DESC";
                    break;
                default:
                    $query = "SELECT estado_bodega.bodega, nombre AS linea,SUM(stock) AS stock FROM estado_bodega left join linea on linea=linea.id WHERE estado_bodega.bodega=$bodega AND (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) $estilo_str group by linea;";
                    break;
            }
        } else {
            switch ($tipoQuery) {
                case 1:
                    $query = "SELECT  linea.id as id_linea, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) WHERE (articulo.linea >= $Li AND articulo.linea<= $Ls) $estilo_str GROUP BY linea.id ORDER BY no DESC";
                    break;
                case 2:
                    $query = "SELECT  linea.id as id_linea, proveedor.nombre as proveedor, proveedor.id as id_proveedor, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id WHERE (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id ORDER BY no DESC";
                    break;
                case 3:
                    $query = "SELECT  articulo.estilo as estilo, linea.id as id_linea, proveedor.nombre as proveedor, proveedor.id as id_proveedor, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id WHERE (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id, articulo.estilo ORDER BY no DESC";
                    break;
                case 4:
                    $query = "SELECT  color.id as id_color, color.nombre as color, articulo.estilo as estilo, linea.id as id_linea, proveedor.nombre as proveedor,proveedor.id as id_proveedor, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id LEFT JOIN color ON articulo.color = color.id WHERE (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id, articulo.estilo, articulo.color ORDER BY no DESC;";
                    break;
                case 5:
                    $query = "SELECT  exi_costo_unitario as costo, articulo.talla as talla, color.nombre as color,color.id as id_color, articulo.estilo as estilo, linea.id as id_linea, proveedor.nombre as proveedor, proveedor.id as id_proveedor, linea.nombre as linea, bodega.nombre as nombre_bodega, bodega, (SUM(ent_cantidad) - SUM(sal_cantidad)) as stock FROM kardex LEFT JOIN articulo ON codigo=articulo.id LEFT JOIN bodega ON bodega.id = bodega LEFT JOIN control_precio c ON (c.control_estilo = articulo.estilo AND c.linea = articulo.linea AND c.color = articulo.color AND c.talla = articulo.talla) LEFT JOIN linea on (articulo.linea = linea.id) LEFT JOIN producto ON (producto.estilo = articulo.estilo AND producto.linea = articulo.linea) LEFT JOIN proveedor ON producto.proveedor = proveedor.id LEFT JOIN color ON articulo.color = color.id WHERE (articulo.linea >= $Li AND articulo.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) $estilo_str GROUP BY linea.id, proveedor.id, articulo.estilo, articulo.color, articulo.talla ORDER BY no DESC";
                    break;
                default:
                    $query = "SELECT estado_bodega.bodega, nombre AS linea,SUM(stock) AS stock FROM estado_bodega left join linea on linea=linea.id WHERE (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) $estilo_str group by linea;";
                    break;
            }
        }
    ...
    public function salidas($id_ref, $bodega_origen, $bodega_destino) {
        ...
        /* Obtiene cada producto dentro del detalle del traslado */
        foreach ($productos as &$producto) {
            $linea = $producto['linea'];
            $estilo = $producto['estilo'];
            $color = $producto['color'];
