...
class inventarioModel extends object {
    ...
    public function c_general_producto($estilo){
        $query = "SELECT linea.id as id_linea, catalogo.nombre as catalogo, linea.nombre as linea, n_pagina,  grupo.nombre as grupo FROM producto LEFT JOIN linea on linea.id = producto.linea INNER JOIN catalogo on catalogo.id = producto.catalogo LEFT JOIN grupo on grupo.id = producto.grupo  WHERE estilo='{$estilo}'";
        data_model()->executeQuery($query);
    ...
    public function consultar_inventario($tipoQuery, $bodega, $Li, $Ls, $Pi, $Ps) {
        ...
        if ($bodega > 0) {
            switch ($tipoQuery) {
                case 1:
                    $query = "SELECT estado_bodega.bodega, nombre AS linea,SUM(stock) AS stock FROM estado_bodega left join linea on linea=linea.id WHERE estado_bodega.bodega=$bodega AND (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) group by linea;";
                    break;
                case 2:
                    $query = "SELECT estado_bodega.bodega, linea.nombre AS linea, linea.id as id_linea,proveedor.nombre AS proveedor,SUM(stock) as stock FROM estado_bodega left join producto on (estado_bodega.estilo = producto.estilo and estado_bodega.linea = producto.linea) left join proveedor on proveedor.id=proveedor left join linea on estado_bodega.linea=linea.id WHERE estado_bodega.bodega=$bodega AND (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) group by proveedor,estado_bodega.linea;";
                    break;
                case 3:
                    ...
                    break;
                case 4:
                    $query = "SELECT estado_bodega.bodega, color.nombre AS color, color.id as id_color,estado_bodega.estilo AS estilo,linea.nombre AS linea, linea.id as id_linea,proveedor.nombre AS proveedor,SUM(stock) AS stock from estado_bodega left join producto on (estado_bodega.estilo = producto.estilo and estado_bodega.linea = producto.linea) left join proveedor on proveedor.id=proveedor left join linea on estado_bodega.linea=linea.id left join color on color.id=estado_bodega.color WHERE estado_bodega.bodega=$bodega AND (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps)  group by proveedor,estado_bodega.linea,estado_bodega.estilo,estado_bodega.color;";
                    break;
                case 5:
                    $query = "SELECT estado_bodega.bodega, estado_bodega.talla AS talla,color.nombre AS color, color.id as id_color,estado_bodega.estilo AS estilo,linea.nombre AS linea, linea.id as id_linea,proveedor.nombre AS proveedor,SUM(estado_bodega.stock) AS stock,costo, fisico from estado_bodega left join control_precio on(estado_bodega.estilo=control_precio.control_estilo AND estado_bodega.linea=control_precio.linea AND estado_bodega.color = control_precio.color AND estado_bodega.talla = control_precio.talla) left join teorico_fisico on(estado_bodega.estilo=teorico_fisico.estilo AND estado_bodega.linea=teorico_fisico.linea AND estado_bodega.color = teorico_fisico.color AND estado_bodega.talla = teorico_fisico.talla) left join producto on (estado_bodega.estilo = producto.estilo and estado_bodega.linea = producto.linea) left join proveedor on proveedor.id=proveedor left join linea on estado_bodega.linea=linea.id left join color on color.id=estado_bodega.color WHERE estado_bodega.bodega=$bodega AND periodo_actual = $pa AND mes=MONTH(CURDATE()) AND (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) group by proveedor,estado_bodega.linea,estado_bodega.estilo,estado_bodega.color,estado_bodega.talla;";
                    break;
                default:
                    $query = "SELECT estado_bodega.bodega, nombre AS linea,SUM(stock) AS stock FROM estado_bodega left join linea on linea=linea.id WHERE estado_bodega.bodega=$bodega AND (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) group by linea;";
                    break;
            ...
            switch ($tipoQuery) {
                case 1:
                    $query = "SELECT estado_bodega.bodega, nombre AS linea,SUM(stock) AS stock FROM estado_bodega left join linea on linea=linea.id WHERE (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) group by linea;";
                    break;
                case 2:
                    $query = "SELECT estado_bodega.bodega, linea.nombre AS linea, linea.id as id_linea,proveedor.nombre AS proveedor,SUM(stock) as stock FROM estado_bodega left join producto on (estado_bodega.estilo = producto.estilo and estado_bodega.linea = producto.linea) left join proveedor on proveedor.id=proveedor left join linea on estado_bodega.linea=linea.id WHERE (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) group by proveedor,estado_bodega.linea;";
                    break;
                case 3:
                    $query = "SELECT estado_bodega.bodega, estado_bodega.estilo AS estilo,linea.nombre AS linea, linea.id as id_linea,proveedor.nombre AS proveedor,SUM(stock) AS stock from estado_bodega left join producto on (estado_bodega.estilo = producto.estilo and estado_bodega.linea = producto.linea) left join proveedor on proveedor.id=proveedor left join linea on estado_bodega.linea=linea.id WHERE (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) group by proveedor,estado_bodega.linea,estado_bodega.estilo;";
                    break;
                case 4:
                    $query = "SELECT estado_bodega.bodega, color.nombre AS color, color.id as id_color,estado_bodega.estilo AS estilo,linea.nombre AS linea, linea.id as id_linea,proveedor.nombre AS proveedor,SUM(stock) AS stock from estado_bodega left join producto on (estado_bodega.estilo = producto.estilo and estado_bodega.linea = producto.linea) left join proveedor on proveedor.id=proveedor left join linea on estado_bodega.linea=linea.id left join color on color.id=estado_bodega.color WHERE (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps)  group by proveedor,estado_bodega.linea,estado_bodega.estilo,estado_bodega.color;";
                    break;
                case 5:
                    $query = "SELECT estado_bodega.bodega, estado_bodega.talla AS talla,color.nombre AS color, color.id as id_color,estado_bodega.estilo AS estilo,linea.nombre AS linea, linea.id as id_linea,proveedor.nombre AS proveedor,SUM(estado_bodega.stock) AS stock,costo from estado_bodega left join control_precio on(estado_bodega.estilo=control_precio.control_estilo AND estado_bodega.linea=control_precio.linea AND estado_bodega.color = control_precio.color AND estado_bodega.talla = control_precio.talla) left join producto on (estado_bodega.estilo = producto.estilo and estado_bodega.linea = producto.linea) left join proveedor on proveedor.id=proveedor left join linea on estado_bodega.linea=linea.id left join color on color.id=estado_bodega.color WHERE (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) AND (producto.proveedor >= $Pi AND producto.proveedor<= $Ps) group by proveedor,estado_bodega.linea,estado_bodega.estilo,estado_bodega.color,estado_bodega.talla;";
                    break;
                default:
                    $query = "SELECT estado_bodega.bodega, nombre AS linea,SUM(stock) AS stock FROM estado_bodega left join linea on linea=linea.id WHERE (estado_bodega.linea >= $Li AND estado_bodega.linea<= $Ls) group by linea;";
                    break;
